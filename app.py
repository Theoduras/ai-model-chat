...
def _threads_media_item(spec):
    ...
    Accepts a bare URL string or a dict with `url` (or `image_url`/`video_url`),
    an optional `type` ("IMAGE"/"VIDEO") and an optional `alt_text`.
    """
    ...
    if mtype not in ('IMAGE', 'VIDEO'):
        ...
        mtype = 'VIDEO' if path.endswith(THREADS_VIDEO_EXTS) or spec.get('video_url') else 'IMAGE'
...
def _threads_await_container(persona, creation_id, timeout=120):
    """Media containers are processed asynchronously and publishing one before
    it reports FINISHED fails, so wait it out with a backing-off poll."""
    ...
    while True:
        ...
        if status in ('ERROR', 'EXPIRED'):
            detail = res.get('error_message') or 'no detail given'
            raise RuntimeError(f'Threads media {status.lower()}: {detail}')
        remaining = deadline - time.time()
        if remaining <= 0:
            raise RuntimeError('Threads is still processing the media after '
                               f'{timeout}s. It may still publish — check the account.')
...
def _threads_publish(persona, text, reply_to_id=None, media=None, alt_text=None):
    ...
    `media` is a list of URLs or media dicts (see `_threads_media_item`); one
    item posts as a single image/video, several as a carousel, none as text.
    """
    uid = _threads_uid(persona)
